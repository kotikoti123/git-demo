...
using System.Linq;
using System.Text;
using System.Data.SqlClient;//install the  package  from nuget manager
    ...
    class ConnectedArchitecture
    {
        static void Main()
        {
            ...
            try
            {
                ...
                int i = cmd.ExecuteNonQuery();
                Console.WriteLine("Records updayed is{0}", i);
                cmd.Parameters.Clear();*/
                //scalar value
                cmd = new SqlCommand("select count(ShipperId) from Shippers", con);
                Console.WriteLine("No of Shipper:{0}", cmd.ExecuteScalar());//return integer

                cmd = new SqlCommand("select CompanyName from Shippers where Shipperid=1", con);
                Console.WriteLine("CompanyName:{0}", cmd.ExecuteScalar());//return string
